...
export const metadata: Metadata = buildPageMetadata({
  title: 'リンク集',
  description: 'SNSアカウントやプロジェクトなど、岡田 龍樹に関連する外部リンクをまとめています。',
  locale: 'ja',
  path: '/ja/links/',
  languageAlternates: {
    'ja-JP': '/ja/links/',
...
export default async function LinksJa() {
  const links = await getLinks();
  const groups = groupBy(links, (l) => l.category || 'その他');
  const mobileLimit = 3;
  ...
  return (
    <div className="space-y-6">
      <div className="text-sm opacity-70">🏠 Home / 🔗 リンク</div>
      <h1 className="text-3xl font-bold">🔗 リンク</h1>
        ...
        <section key={cat} className="space-y-2">
          ...
          {items.length > mobileLimit ? (
            <details className="sm:hidden">
              <summary className="cursor-pointer text-sm underline">さらに表示</summary>
